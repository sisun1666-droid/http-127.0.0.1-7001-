﻿﻿
    ...
    function authMsg(msg,isError=false){const el=$("#authMessage");if(el){el.textContent=msg;el.style.color=isError?"#dc2626":""}}
    function showAuthGate(msg="직원 이름과 개인 PIN을 입력해주세요."){document.body.classList.add("auth-pending");authMsg(msg)}
    function showApp(){authReady=true;loadSharedState(false,true).finally(()=>{if(!authUser)return;try{render();}catch(e){console.warn("showApp render error",e);}ensureSheetSyncButton();document.body.classList.remove("auth-pending");
    ...
    function showLoginError(msg){setLoginButtonState(false);authMsg(msg,true);const card=$(".auth-card");card?.classList.remove("auth-error");void card?.offsetWidth;card?.classList.add("auth-error")}
    function staffLogin(){
    ...
    async function initAuthGate(){
      ensureDefaultStaffPins();
      const person=state.people.find(p=>p.name==="이재강")||fallbackStaff[0];
      setStaffSession(person);
    ...
    async function saveDataToSupabase(data=state,options={}){
      ...
      ["__lastSavedAt","__lastSavedAtText","__deviceId","__updatedAt"].forEach(k=>{if(data[k]!==undefined)cfgData[k]=data[k]});const _tbData={};TABLE_KEYS.forEach(t=>{const e={};_deletedTombstones[t].forEach((ts,id)=>{e[id]=ts;});if(Object.keys(e).length)_tbData[t]=e;});if(Object.keys(_tbData).length)cfgData.__tombstones=_tbData;
    ...
    async function pushSharedState(){if(syncSaveBusy){syncSaveQueued=true;return false}syncSaveBusy=true;try{
  /* 서버가 더 최신이면 덮어쓰지 않고 서버 데이터 적용 */
  try{const _srv=await loadSupabaseData();if(_srv&&syncStamp(_srv)>syncStamp(state)){applySharedState(_srv,false);setSyncNotice("ok","더 최신 데이터가 서버에 있어 불러왔습니다.");return true;}}catch(_e){setSyncNotice("warn","서버 상태 확인 실패. 30초 후 재시도합니다.");return false;}
  const expectedAt=new Date().toISOString();state.__lastSavedAt=expectedAt;state.__lastSavedAtText=new Date().toLocaleString("ko-KR");setSyncNotice("saving","수정 내용을 자동으로 Supabase에 저장하는 중입니다.");await saveToSupabase();delete state.__pendingCloudSync;delete state.__pendingCloudSyncAt;localStorage.setItem(storageKey,JSON.stringify(state));const verified=await verifySupabaseSave(expectedAt);setSyncNotice(verified?"ok":"warn",verified?"자동 저장이 완료되었습니다. 다른 컴퓨터에서도 최신 내용을 불러옵니다.":"저장 요청은 보냈지만 Supabase에서 같은 저장시각을 다시 확인하지 못했습니다. 네트워크 또는 권한을 확인해주세요.");return verified}catch(err){state.__pendingCloudSync=true;state.__pendingCloudSyncAt=new Date().toLocaleString("ko-KR");localStorage.setItem(storageKey,JSON.stringify(state));setSyncNotice("warn",`자동 저장 실패: ${err?.message||"확인 필요"} 현재 데이터는 이 브라우저에 대기 저장되었습니다.`);return false}finally{syncSaveBusy=false;if(syncSaveQueued){syncSaveQueued=false;scheduleSharedSave(250)}else if(state.__pendingCloudSync){scheduleSharedSave(30000)}}}
...
function inspRows(b) {
  return [['발전소명','주소','용량(kW)','외주업체','계약금','검수일','검수자','결과','ID'],
    ...
    function applySharedState(shared,forceReplace=false){
      ...
      const merged={...clone(defaults),...shared,nav:clone(defaults.nav)};if(shared.__tombstones){const _now=Date.now();TABLE_KEYS.forEach(t=>{if(shared.__tombstones[t])Object.entries(shared.__tombstones[t]).forEach(([id,ts])=>{if(_now-ts<_tombstoneTtl&&(!_deletedTombstones[t].has(id)||_deletedTombstones[t].get(id)<ts)){_deletedTombstones[t].set(id,ts);}});});saveTombstones();}
  ...
  state.todos.forEach(t=>{
    ...
    renderTodoBoard=function(){ensureTaskLinks();const panel=$("#todoBoardPanel"),statuses=["백로그","할 일","진행중","완료","취소"],q=$("#todoSearch")?.value||"",owners=["전체",...state.people.map(p=>p.name)],filtered=state.todos.map((t,i)=>({t,i})).filter(({t})=>(todoStatusFilter==="전체"||t.status===todoStatusFilter)&&(todoOwnerFilter==="전체"||t.owner===todoOwnerFilter)&&todoMatches(t,q));const chips=`<div class="todo-chips"><button class="todo-chip ${todoStatusFilter==="전체"?"active":""}" data-todo-status-filter="전체">전체 <strong>${state.todos.length}</strong></button>${statuses.map(s=>`<button class="todo-chip ${todoStatusFilter===s?"active":""}" data-todo-status-filter="${esc(s)}">${esc(s)} <strong>${state.todos.filter(t=>t.status===s).length}</strong></button>`).join("")}</div><div class="todo-chips">${owners.map(o=>`<button class="todo-chip ${todoOwnerFilter===o?"active":""}" data-todo-owner-filter="${esc(o)}">${esc(o)} <strong>${o==="전체"?state.todos.length:state.todos.filter(t=>t.owner===o).length}</strong></button>`).join("")}</div>`;const board=`<div class="todo-board">${statuses.map(s=>{const rows=filtered.filter(x=>x.t.status===s);const colStyle={bg:{백로그:"#f5f3ff","할 일":"#fffbeb","진행중":"#eff6ff","완료":"#f0fdf4","취소":"#fef2f2"},bd:{백로그:"#c4b5fd","할 일":"#fcd34d","진행중":"#93c5fd","완료":"#86efac","취소":"#fca5a5"},hbg:{백로그:"#ede9fe","할 일":"#fef3c7","진행중":"#dbeafe","완료":"#dcfce7","취소":"#fee2e2"},htx:{백로그:"#5b21b6","할 일":"#92400e","진행중":"#1e40af","완료":"#15803d","취소":"#991b1b"},cbr:{백로그:"#8b5cf6","할 일":"#f59e0b","진행중":"#3b82f6","완료":"#22c55e","취소":"#ef4444"}};return`<div class="todo-column" style="background:${colStyle.bg[s]||"#f8fafc"};border:1.5px solid ${colStyle.bd[s]||"#e2e8f0"};border-radius:11px;padding:9px;min-height:260px"><div class="todo-column-head" style="background:${colStyle.hbg[s]||"#f1f5f9"};color:${colStyle.htx[s]||"#1e293b"};border-radius:7px;padding:4px 7px;font-weight:900;font-size:12.5px;display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><span>${esc(s)}</span><span style="font-size:11px;background:rgba(0,0,0,.12);padding:1px 6px;border-radius:999px">${rows.length}</span></div>${rows.length?rows.map(({t,i})=>`<div class="todo-card" style="border-left:4px solid ${colStyle.cbr[t.status]||"#cbd5e1"};border-radius:8px;background:#fff;padding:8px 9px;margin-bottom:6px;border:1px solid #e2e8f0;border-left-width:4px;box-shadow:0 1px 3px rgba(0,0,0,.05)"><div style="font-weight:700;font-size:12.5px;line-height:1.35;margin-bottom:4px">${esc(t.title)}</div><div style="color:#64748b;font-size:11px;line-height:1.5">${esc(t.owner||"담당 미정")} · ${esc(t.priority||"보통")} · ${esc(t.due||"")}</div>${t.detail?`<div style="color:#64748b;font-size:11px;margin-top:2px">${esc(t.detail.length>40?t.detail.slice(0,40)+"…":t.detail)}</div>`:""}<div class="row-actions" style="margin-top:7px"><button class="btn icon" data-edit-todo="${i}">✎</button><button class="btn icon danger" data-delete-todo="${i}">×</button></div></div>`).join(""):`<div class="meta" style="font-size:12px;padding:8px 4px">비어 있음</div>`}<button class="btn" style="width:100%;font-size:12px;margin-top:4px" data-add-todo-status="${esc(s)}">+ 빠른 추가</button></div>`}).join("")}</div>`;panel.innerHTML=`<div class="todo-toolbar"><div class="todo-view"><button class="${todoViewMode==="board"?"active":""}" data-todo-view="board">보드</button><button class="${todoViewMode==="list"?"active":""}" data-todo-view="list">목록</button></div><button class="btn primary" id="todoAddBtn">할일 추가</button></div><div class="todo-filters"><input class="search" id="todoSearch" placeholder="제목, 설명, 담당자, 우선순위 검색" value="${esc(q)}">${chips}</div>${todoViewMode==="list"?todoListHtml(filtered):board}`;$("#todoSearch")?.addEventListener("input",renderTodoBoard)};
  ...
  if(e.target.id==="importJsonInput"&&e.target.files[0]){importStateJson(e.target.files[0]);e.target.value=""}
    ...
    (function setupFieldworkView(){
      ...
      function ensureFieldworkState(){
        ...
        if(!state.nav.some(n=>n.label==="일정관리"||n.label==="외근현황")){
          const adminIndex=state.nav.findIndex(n=>n.label.includes("관리자"));
          state.nav.splice(adminIndex>=0?adminIndex:state.nav.length,0,{icon:"📅",label:"일정관리"});
        }
        state.nav=state.nav.map(n=>n.label==="외근현황"?{...n,icon:"📅",label:"일정관리"}:n);
      }
      ...
      viewForLabel=function(label){return label.includes("일정관리")||label.includes("외근현황")||label.includes("외근")?"fieldwork":baseViewForFieldwork(label)};
      const baseIsActiveForFieldwork=isActive;
      isActive=function(label){return currentView==="fieldwork"?(label.includes("일정관리")||label.includes("외근현황")):baseIsActiveForFieldwork(label)};
      ...
      renderView=function(){
        ...
        els.mainGrid?.classList.toggle("hidden",(currentView==="dashboard"||currentView==="admin")||isField);
        document.getElementById('schedView')?.classList.toggle("hidden",!isField);
        els.fieldworkView?.classList.add("hidden");
      ...
      renderCurrentContent=function(){if(currentView==="fieldwork"){syncViewChrome();renderSchedule();return}baseRenderCurrentForFieldwork()};
      const baseSyncForFieldwork=syncViewChrome;
      syncViewChrome=function(){baseSyncForFieldwork();if(currentView==="fieldwork"){els.pageTitle.textContent="일정관리";els.pageSub.textContent="월별 일정 캘린더 · 외근/시공/할일 통합 관리";if($("#addProjectBtn"))$("#addProjectBtn").style.display="none"}};
    ...
    (function improveFieldworkWorkflow(){
      ...
      renderCurrentContent=function(){if(currentView==="fieldwork"){renderSchedule();return}baseRenderCurrentForWorkflow()};
      const baseSyncViewForWorkflow=syncViewChrome;
      syncViewChrome=function(){baseSyncViewForWorkflow();if(currentView==="fieldwork"){els.pageTitle.textContent="일정관리";els.pageSub.textContent="월별 일정 캘린더 · 외근/시공/할일 통합 관리";const topAction=$("#addProjectBtn");if(topAction)topAction.style.display="none"}};
      document.addEventListener("click",e=>{
        const t=e.target.closest("button")||e.target;
    ...
    (function setupMeetingMinutes(){
      ...
      function meetingAiSection(text,label){
        ...
        const prefix=`(?:#{1,3}\\s*|\\*{1,2})?`;
        const suffix=`(?:\\*{1,2})?`;
        // [:：] 뒤 \*{0,2} 로 "**레이블:**" 형식(콜론 뒤 닫는 ** 포함) 처리
        const re=new RegExp(`(?:^|\\n)[^\\S\\n]*${prefix}(?:${safeLabel})${suffix}[^\\S\\n]*[:：]\\*{0,2}[^\\S\\n]*([\\s\\S]*?)(?=\\n[^\\S\\n]*${prefix}(?:${labels})${suffix}[^\\S\\n]*[:：]|$)`,"i");
      ...
      function parseMeetingRawText(text){
        const lines=text.split(/\r?\n/);
        const firstLine=(lines[0]||"").trim();
        const date=parseMeetingDate(firstLine);
        const title=firstLine.replace(/\d{1,2}[\/.\-월]\d{1,2}[일]?\s*/,"").trim()||firstLine;
        const decisions=lines.slice(1).map(l=>l.trim()).filter(Boolean);
        return {title,date,time:"",host:"",attendees:knownMeetingPeople(text),project:"",type:"",summary:"",decisions}
      }
      function parseMeetingAiText(text){
        ...
        if(!d.title&&!d.date&&!d.summary&&!d.decisions.length)return parseMeetingRawText(text);
        return d;
      }
      function applyMeetingAiText(){
        ...
        let filled=0;
        if(data.title&&$("#meetingTitle")){$("#meetingTitle").value=data.title;filled++}
        if(data.date&&$("#meetingDate")){$("#meetingDate").value=data.date;filled++}
        if(data.time&&$("#meetingTime")){$("#meetingTime").value=data.time;filled++}
        if(data.host&&$("#meetingHost")){$("#meetingHost").value=data.host;if($("#meetingHost").value!==data.host){const matched=(state.people||[]).find(p=>data.host.includes(p.name));if(matched)$("#meetingHost").value=matched.name}filled++}
        if(data.type&&$("#meetingType")){$("#meetingType").value=data.type;filled++}
        if(data.attendees.length){renderMeetingAttendeeChecks(data.attendees);filled++}
        if(data.project&&$("#meetingProject")){$("#meetingProject").value=data.project;filled++}
        if(data.summary&&$("#meetingSummary")){$("#meetingSummary").value=data.summary;filled++}
        if(data.decisions.length&&$("#meetingDecisions")){$("#meetingDecisions").value=data.decisions.join("\n");filled++}
        if(filled===0){
          toast("인식된 항목이 없습니다. '레이블: 값' 형식으로 붙여넣어 주세요.");
          return;
        }
        toast(`${filled}개 항목을 회의록에 자동 입력했습니다.`);
      }
      ...
      async function transcribeAudioWithGemini(file){
        ...
        try{
          ...
          const resp=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent`,{method:"POST",headers:{"Content-Type":"application/json","x-goog-api-key":key},body:JSON.stringify({contents:[{parts:[contentPart,{text:prompt}]}],generationConfig:{temperature:0.1}})});
      ...
      const rows=(state.people||[]).map((p,i)=>`
        <div class="auth-row" style="flex-wrap:wrap;gap:8px;align-items:center">
          ...
          <span class="row-actions" style="flex-wrap:wrap;gap:6px">
            <input class="field" type="password" data-admin-person-pin="${i}" value="${esc(staffPin(p)||"1234")}" placeholder="개인 PIN" style="width:90px;min-height:30px">
    ...
    (function setupMessageReplyCenter(){
      /* 시공 단계별 일정 답변 문구 (정확한 날짜 없이 진행 상황을 교묘하게 전달) */
      const schedulePhaseReply={
        "고객상담":  "현재 현장 검토 및 사업 가능성 확인 단계를 진행 중입니다. 검토가 마무리되는 대로 전체 진행 일정을 정리해 안내드리겠습니다.",
        "현장조사":  "현재 현장 실측·조사가 진행 중입니다. 조사 완료 후 설계 방향과 이후 일정을 구체적으로 안내드리겠습니다.",
        "인허가":    "현재 관청 인허가 절차를 진행하고 있습니다. 허가 승인이 나는 즉시 착공 일정을 잡아 사전에 연락드리겠습니다.",
        "한전접수":  "현재 한전 계통 연계 접수가 진행 중입니다. 한전 측 검토가 완료되는 대로 시공 일정을 확정해 안내드리겠습니다.",
        "설계검토":  "현재 현장 맞춤 설계를 최종 검토하는 단계입니다. 검토가 마무리되면 바로 자재 준비와 착공 일정을 잡겠습니다.",
        "자재발주":  "현재 시공에 필요한 자재를 발주한 상태입니다. 자재 입고가 확인되는 대로 착공 일정을 먼저 연락드리겠습니다.",
        "시공중":    "현재 현장에서 시공이 진행되고 있습니다. 작업 진행 상황에 따라 완료 일정이 조율되고 있으며, 일정 확인 후 안내드리겠습니다.",
        "준공":      "시공이 마무리 단계에 접어들었습니다. 준공 확인과 마무리 서류 처리가 완료되는 대로 관련 내용을 안내드리겠습니다.",
        "완료":      "해당 현장은 시공이 완료된 상태입니다. 문의하신 내용이 추가 사항이라면 담당자가 확인 후 안내드리겠습니다."
      };
      const scheduleStatusReply={
        "예정":   "현재 시공 일정이 확정된 상태로 준비 중입니다. 작업 시작 전 사전에 다시 안내드리겠습니다.",
        "지연":   "현장 여건상 일정이 다소 조율 중에 있습니다. 담당자가 확인 후 변경된 일정을 안내드리겠습니다.",
        "완료":   "해당 현장 시공은 완료된 상태입니다. 추가 사항이 있으시면 담당자가 검토 후 안내드리겠습니다."
      };
      /* 현장명 정규화: "발전소", "태양광", "솔라" 등 일반 접미사 제거 */
      function normalizeSiteKey(name){
        return String(name||"")
          .replace(/\s/g,"")
          .replace(/(발전소|태양광|솔라|solar|pv|발전|사업)/gi,"")
          .toLowerCase();
      }
      /* 두 키 중 하나가 다른 하나를 포함하는지 확인 */
      function siteKeyMatch(a,b){return a&&b&&(a.includes(b)||b.includes(a))}
      /* 현장명으로 시공일정 → 프로젝트 → EPC DB 순서로 검색 */
      function findSiteRecord(siteName){
        if(!siteName)return null;
        const raw=siteName.replace(/\s/g,"").toLowerCase();
        const norm=normalizeSiteKey(siteName);
        /* 1. state.construction (시공일정) */
        const constr=(state.construction||[]).find(c=>{
          const s=String(c.site||c.name||"").replace(/\s/g,"").toLowerCase();
          const sn=normalizeSiteKey(c.site||c.name||"");
          return siteKeyMatch(s,raw)||siteKeyMatch(sn,norm);
        });
        if(constr)return{source:"construction",data:constr};
        /* 2. state.projects (현장·공무 진행표) */
        const proj=(state.projects||[]).find(p=>{
          const s=String(p.name||"").replace(/\s/g,"").toLowerCase();
          const sn=normalizeSiteKey(p.name||"");
          return siteKeyMatch(s,raw)||siteKeyMatch(sn,norm);
        });
        if(proj)return{source:"project",data:proj};
        /* 3. SOLAR_EPC_DATA (엑셀 EPC DB) */
        const epcList=(window.SOLAR_EPC_DATA&&window.SOLAR_EPC_DATA.projects)||[];
        const epc=epcList.find(p=>{
          const s=String(p.plant||"").replace(/\s/g,"").toLowerCase();
          const sn=normalizeSiteKey(p.plant||"");
          return siteKeyMatch(s,raw)||siteKeyMatch(sn,norm);
        });
        if(epc)return{source:"epc",data:epc};
        return null;
      }
      /* EPC 상태값 → 단계 매핑 */
      const epcStatusToPhase={
        "공사중":"시공중","시공중":"시공중","건설중":"시공중",
        "자재발주":"자재발주","자재준비":"자재발주",
        "설계중":"설계검토","설계검토":"설계검토",
        "인허가":"인허가","허가중":"인허가","허가진행":"인허가",
        "한전접수":"한전접수","한전진행":"한전접수",
        "현장조사":"현장조사","조사중":"현장조사",
        "건설완료":"완료","준공완료":"완료","완료":"완료","공사완료":"완료",
        "준공":"준공","마무리":"준공",
        "계약":"고객상담","상담중":"고객상담","진행중":"시공중"
      };
      /* 현장 데이터 기반 일정 답변 생성 */
      function buildScheduleReplyFromSite(siteName){
        const found=findSiteRecord(siteName);
        if(!found)return null;
        const {source,data}=found;
        if(source==="construction"){
          const phase=data.phase||data.constructionPhase||"";
          const status=data.status||"";
          if(scheduleStatusReply[status])return scheduleStatusReply[status];
          if(schedulePhaseReply[phase])return schedulePhaseReply[phase];
          if(data.start||data.end){
            const parts=[];
            if(data.start)parts.push(`시작: ${data.start}`);
            if(data.end)parts.push(`예상 완료: ${data.end}`);
            return `현장 시공 일정을 확인 중입니다. (${parts.join(" / ")}) 상세 일정은 담당자가 확인 후 안내드리겠습니다.`;
          }
        }
        if(source==="project"){
          const phase=data.phase||"";
          if(schedulePhaseReply[phase])return schedulePhaseReply[phase];
          if(data.due)return `현재 ${phase||"진행"} 단계로, 예정 기한(${data.due}) 맞춰 진행 중입니다. 상세 일정은 담당자가 확인 후 안내드리겠습니다.`;
        }
        if(source==="epc"){
          const rawStatus=String(data.status||"").toLowerCase();
          const phase=epcStatusToPhase[data.status]||epcStatusToPhase[rawStatus]||"";
          if(schedulePhaseReply[phase])return schedulePhaseReply[phase];
          if(scheduleStatusReply[data.status])return scheduleStatusReply[data.status];
          return `현재 해당 현장의 진행 상황을 확인하고 있습니다. 담당자가 일정을 검토한 뒤 안내드리겠습니다.`;
        }
        return null;
      }
      const messageCategories=[
        ...
        {type:"일정 문의",badge:"blue",keys:["언제","일정","방문","착공","준공","공사","시공","완료","도착","몇시","몇 시","늦","지연","시작","시작해","올건가","올건지","합니까","언제쯤","올예정"],reply:"일정 관련 문의 확인했습니다. 현재 진행 단계와 담당자 일정, 현장 여건을 함께 확인한 뒤 가능한 일정으로 안내드리겠습니다."},
      ...
      function buildMessageReply(){
        ...
        /* 일정 문의이고 현장명이 있으면 DB 조회해서 단계별 맞춤 답변 생성 */
        if(a.type==="일정 문의"&&site){
          const siteReply=buildScheduleReplyFromSite(site);
          lines.push(siteReply||a.reply);
          /* 현장 정보 발견 시 내부 참고 안내 (※ 줄은 실제 답변에 포함되지 않음) */
          const found=findSiteRecord(site);
          if(found){
            const d=found.data;
            const phase=d.phase||d.constructionPhase||(found.source==="epc"?epcStatusToPhase[d.status]:"")|| "";
            const status=d.status||"";
            const info=[];
            if(phase&&phase!==status)info.push(`단계: ${phase}`);
            if(status)info.push(`상태: ${status}`);
            if(d.start&&(d.end||d.due))info.push(`기간: ${d.start} ~ ${d.end||d.due}`);
            else if(d.start)info.push(`시작: ${d.start}`);
            else if(d.end||d.due)info.push(`완료예정: ${d.end||d.due}`);
            if(d.capacity||d.kw)info.push(`용량: ${d.capacity||d.kw}kW`);
            if(info.length){lines.push("");lines.push(`【내부참고】 ${info.join(" / ")}`);}
          }
        }else{
          lines.push(a.reply);
        }
      ...
      function buildSiteInfoHtml(siteName){
        if(!siteName)return"";
        const found=findSiteRecord(siteName);
        if(!found)return`<div id="messageSiteInfo" style="background:#fef9ec;border:1px solid #fde68a;border-radius:8px;padding:10px 14px;margin-top:4px;font-size:12px;color:#92400e">⚠️ DB에서 <strong>${esc(siteName)}</strong> 현장을 찾지 못했습니다. 현장명을 다시 확인해 주세요.</div>`;
        const d=found.data;
        const sourceLabel={construction:"시공일정",project:"프로젝트",epc:"EPC DB"}[found.source]||"DB";
        const displayName=d.plant||d.site||d.name||siteName;
        /* 소스별 필드 통합 */
        const phase=d.phase||d.constructionPhase||(found.source==="epc"?epcStatusToPhase[d.status]:"")|| "";
        const status=d.status||"";
        const startDate=d.start||d.contractDate||"";
        const endDate=d.end||d.due||"";
        const owner=d.owner||d.manager||"";
        const region=d.region||d.location||"";
        const capacity=d.capacity||d.kw||"";
        const chips=[];
        if(phase&&phase!==status)chips.push(`<span style="background:#dbeafe;color:#1e40af;padding:2px 8px;border-radius:12px;font-size:11px">${esc(phase)}</span>`);
        if(status)chips.push(`<span style="background:#d1fae5;color:#065f46;padding:2px 8px;border-radius:12px;font-size:11px">${esc(status)}</span>`);
        if(startDate||endDate)chips.push(`<span style="background:#ede9fe;color:#4c1d95;padding:2px 8px;border-radius:12px;font-size:11px">${startDate||""}${startDate&&endDate?" ~ ":""}${endDate||""}</span>`);
        if(capacity)chips.push(`<span style="background:#fff7ed;color:#c2410c;padding:2px 8px;border-radius:12px;font-size:11px">${capacity}kW</span>`);
        if(owner)chips.push(`<span style="background:#f3f4f6;color:#374151;padding:2px 8px;border-radius:12px;font-size:11px">담당: ${esc(owner)}</span>`);
        if(region)chips.push(`<span style="background:#f3f4f6;color:#374151;padding:2px 8px;border-radius:12px;font-size:11px">${esc(region)}</span>`);
        return`<div id="messageSiteInfo" style="background:#f0fdf4;border:1px solid #86efac;border-radius:8px;padding:10px 14px;margin-top:4px;font-size:12px"><span style="color:#15803d;font-weight:600">✅ ${esc(sourceLabel)} 조회됨</span> · <strong>${esc(displayName)}</strong><div style="margin-top:6px;display:flex;flex-wrap:wrap;gap:4px">${chips.join("")}</div></div>`;
      }
      ...
      function updateMessagePreview(){
        ...
        /* 현장 정보 박스 실시간 업데이트 */
        const siteEl=$("#messageSite");
        const siteInfoEl=$("#messageSiteInfo");
        const siteVal=siteEl?.value||"";
        if(siteVal){
          const newHtml=buildSiteInfoHtml(siteVal);
          if(siteInfoEl){siteInfoEl.outerHTML=newHtml||""}
          else if(siteEl&&!siteInfoEl&&newHtml){siteEl.insertAdjacentHTML("afterend",newHtml)}
        }else if(siteInfoEl){siteInfoEl.remove()}
      }
    ...
    (function setupAllocationView(){
      ...
      async function extractAllocFromImage(file){
        ...
        try{
          ...
          const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent`,{method:"POST",headers:{"Content-Type":"application/json","x-goog-api-key":key},body:JSON.stringify(body)});
    ...
    (function setupGoogleCalendarSync(){
      ...
      let accessToken=null,tokenExpiresAt=0,tokenClient=null,autoRefreshTimer=null;
      ...
      /* 앱 공유 상태에서 토큰 복원 (Supabase 통해 모든 기기 공유 — 한 기기에서 연결하면 전체 적용) */
      function loadTokenFromState(){
        if(state.gcalToken&&state.gcalTokenExpiry>Date.now()+5000){
          accessToken=state.gcalToken;tokenExpiresAt=state.gcalTokenExpiry;
          scheduleAutoRefresh(Math.round((tokenExpiresAt-Date.now())/1000));
          return true;
        }
        /* 구버전 localStorage → state 마이그레이션 (최초 1회) */
        try{const d=JSON.parse(localStorage.getItem("gcal_token_v1")||"{}");if(d.t&&d.e>Date.now()+5000){saveToken(d.t,Math.round((d.e-Date.now())/1000));localStorage.removeItem("gcal_token_v1");return true;}}catch(e){}
        return false;
      }
      function saveToken(t,expiresIn){
        accessToken=t;tokenExpiresAt=Date.now()+(expiresIn-60)*1000;
        /* Supabase 공유 상태에 저장 → 팀원/다른 기기도 자동 인식 */
        state.gcalToken=accessToken;state.gcalTokenExpiry=tokenExpiresAt;
        scheduleSharedSave(500);
        scheduleAutoRefresh(expiresIn);
      }
      function clearToken(){
        accessToken=null;tokenExpiresAt=0;
        state.gcalToken="";state.gcalTokenExpiry=0;
        localStorage.removeItem("gcal_token_v1");
        scheduleSharedSave(500);
        if(autoRefreshTimer){clearTimeout(autoRefreshTimer);autoRefreshTimer=null}
      }
      /* 만료 10분 전 자동 갱신 예약 */
      function scheduleAutoRefresh(expiresIn){
        if(autoRefreshTimer){clearTimeout(autoRefreshTimer);autoRefreshTimer=null}
        const delay=Math.max(((expiresIn||3600)-600)*1000,5000);
        autoRefreshTimer=setTimeout(async()=>{
          if(!clientId())return;
          const ok=await requestToken(false);
          if(!ok){clearToken();updateGcalBtn();renderGcalBanner();}
        },delay);
      }
      /* 앱 상태 로드 시 토큰 복원 */
      loadTokenFromState();
      /* GIS(Google Identity Services) 기반 토큰 클라이언트 초기화 */
      function initGisClient(){
        if(!window.google?.accounts?.oauth2||!clientId())return false;
        if(tokenClient&&tokenClient.__cid===clientId())return true;
        tokenClient=google.accounts.oauth2.initTokenClient({
          client_id:clientId(),scope:SCOPE,
          callback:(resp)=>{
            if(resp.access_token){saveToken(resp.access_token,resp.expires_in||3600);updateGcalBtn();if(tokenClient.__resolve){tokenClient.__resolve(true);tokenClient.__resolve=null}}
            else{if(tokenClient.__resolve){tokenClient.__resolve(false);tokenClient.__resolve=null}}
          },
          error_callback:(err)=>{console.warn("GIS:",err);if(tokenClient.__resolve){tokenClient.__resolve(false);tokenClient.__resolve=null}}
        });
        tokenClient.__cid=clientId();
        return true;
      }
      /* 토큰 요청: interactive=false면 조용히(팝업 없이) 갱신 시도 */
      async function requestToken(interactive=true){
        ...
        if(window.google?.accounts?.oauth2){
          if(!initGisClient())return false;
          return new Promise(resolve=>{
            tokenClient.__resolve=resolve;
            try{tokenClient.requestAccessToken({prompt:interactive?"select_account":""})}
            catch(e){resolve(false)}
          });
        }
        /* GIS 미로드 시 직접 팝업 fallback */
        if(!interactive)return false;
        const REDIRECT_URI="https://sisun1666-droid.github.io/http-127.0.0.1-7001-/";
        return new Promise(resolve=>{
          const authUrl="https://accounts.google.com/o/oauth2/v2/auth?client_id="+encodeURIComponent(clientId())+"&redirect_uri="+encodeURIComponent(REDIRECT_URI)+"&response_type=token&scope="+encodeURIComponent(SCOPE)+"&prompt=select_account";
          ...
          const timer=setInterval(()=>{
            try{
              ...
              if(url.startsWith(REDIRECT_URI)||url.startsWith("https://sisun1666-droid.github.io")){
                const hash=popup.location.hash.slice(1);const params=new URLSearchParams(hash);
                const token=params.get("access_token");const exp=parseInt(params.get("expires_in")||"3600");
                popup.close();clearInterval(timer);
                if(token){saveToken(token,exp);updateGcalBtn();resolve(true)}else{resolve(false)}
              }
            }catch(e){}
          },400);
        });
      ...
      window._onGisLoaded=function(){
        if(isConnected()){
          /* Supabase에서 가져온 유효 토큰 있음 → UI만 업데이트하고 자동 pull */
          updateGcalBtn();
          autoPullFromGcal();
          return;
        }
        if(!clientId())return;
        /* 토큰 없거나 만료 → silent re-auth 시도 (구글 로그인 상태면 팝업 없이 자동 갱신) */
        initGisClient();
        setTimeout(()=>{
          if(isConnected()||!clientId())return;
          requestToken(false).then(ok=>{
            if(ok){updateGcalBtn();autoPullFromGcal();}
            else{updateGcalBtn();} /* 조용히 실패 — 사용자가 직접 연결 버튼 누를 때까지 대기 */
          });
        },800);
      };
      /* Supabase 공유 상태 갱신 시 → 다른 기기에서 연결한 토큰 자동 수신 */
      const _baseApplySharedForGcal=applySharedState;
      applySharedState=function(shared,forceReplace){
        _baseApplySharedForGcal(shared,forceReplace);
        if(!isConnected()&&loadTokenFromState()){updateGcalBtn();autoPullFromGcal();}
      };
      /* 페이지 로드 시 만료된 토큰 조용히 정리 */
      if(accessToken&&!isConnected()){clearToken();}
      /* 1분마다 토큰 만료 확인 */
      setInterval(()=>{if(accessToken&&!isConnected()){clearToken();updateGcalBtn();renderGcalBanner();}},60*1000);
      ...
      /* Assignment → Calendar 이벤트 변환 */
      function assignmentToEvent(a){
        const start=a.start||a.due||today,end=a.due||start;
        const colorMap={"긴급":"11","높음":"6","완료":"2","진행중":"7","취소":"8"};
        const colorId=colorMap[a.priority]||colorMap[a.status];
        const desc=[a.detail,a.owner?"담당: "+a.owner:"",a.project?"프로젝트: "+a.project:"",a.result?"✅ 결과: "+a.result:""].filter(Boolean).join("\n");
        const body={summary:a.title||"일정",description:desc,extendedProperties:{private:{kiwoomAssignId:a.id||"",kiwoomStatus:a.status||"",kiwoomOwner:a.owner||""}}};
        if(colorId)body.colorId=colorId;
        if(a.location)body.location=a.location;
        if(a.allDay||!a.startTime){const endD=new Date(end);endD.setDate(endD.getDate()+1);body.start={date:start};body.end={date:endD.toISOString().slice(0,10)}}
        else{const st=a.startTime||"09:00",et=a.endTime||"10:00";body.start={dateTime:`${start}T${st}:00`,timeZone:"Asia/Seoul"};body.end={dateTime:`${end}T${et}:00`,timeZone:"Asia/Seoul"}}
        return body
      }
      /* Calendar API 호출 */
      ...
      async function gcalCreateEvent(body){const ev=await gcalFetch("/calendars/"+encodeURIComponent(calendarId())+"/events",{method:"POST",body:JSON.stringify(body)});return ev?.id}
      async function gcalUpdateEvent(gcalId,body){await gcalFetch(`/calendars/${encodeURIComponent(calendarId())}/events/${gcalId}`,{method:"PUT",body:JSON.stringify(body)})}
      async function gcalDelete(gcalId){await gcalFetch(`/calendars/${encodeURIComponent(calendarId())}/events/${gcalId}`,{method:"DELETE"})}
      /* Todo 동기화 */
      async function syncTodoToGcal(t){if(!isConnected()||!t)return;try{if(t.gcalEventId){await gcalUpdateEvent(t.gcalEventId,todoToEvent(t))}else{const id=await gcalCreateEvent(todoToEvent(t));if(id){t.gcalEventId=id;saveStateAfterPaint()}}}catch(e){console.warn("gcal sync:",e)}}
      async function removeTodoFromGcal(gcalId){if(!isConnected()||!gcalId)return;try{await gcalDelete(gcalId)}catch(e){}}
      /* Assignment 동기화 */
      async function syncAssignmentToGcal(a){if(!isConnected()||!a)return;try{if(a.gcalEventId){await gcalUpdateEvent(a.gcalEventId,assignmentToEvent(a))}else{const id=await gcalCreateEvent(assignmentToEvent(a));if(id){a.gcalEventId=id;saveStateAfterPaint()}}}catch(e){console.warn("gcal assign sync:",e)}}
      async function removeAssignmentFromGcal(gcalId){if(!isConnected()||!gcalId)return;try{await gcalDelete(gcalId)}catch(e){}}
      /* 구글 → 앱 가져오기 */
      function isoFromDate(d){return new Date(d+"T00:00:00+09:00").toISOString()}
      function todayStartISO(){return isoFromDate(today)}
      async function pullFromGcal(fromDate){
        ...
        try{
          const timeMin=fromDate?isoFromDate(fromDate):todayStartISO();
          const data=await gcalFetch(`/calendars/${encodeURIComponent(calendarId())}/events?timeMin=${timeMin}&maxResults=300&singleEvents=true&orderBy=startTime&showDeleted=false`);
      ...
      /* 날짜 범위 선택 가져오기 */
      function pullWithDatePicker(){
        const d30=new Date();d30.setDate(d30.getDate()-30);
        const defaultFrom=localDateString(d30);
        const from=prompt(`가져올 시작 날짜를 입력하세요 (YYYY-MM-DD)\n예) 30일 전부터: ${defaultFrom}`,defaultFrom);
        if(from===null)return;
        pullFromGcal(from.trim()||defaultFrom);
      }
      /* 전체 할일 → 구글 캘린더 업로드 */
      ...
      /* 전체 일정 → 구글 캘린더 업로드 */
      async function pushAllAssignmentsToGcal(){
        if(!await requestToken())return;
        const assigns=state.assignments.filter(a=>a.status!=="취소"&&a.status!=="보류");
        toast(`${assigns.length}건 구글 캘린더에 업로드 중...`);
        let ok=0;
        for(const a of assigns){try{await syncAssignmentToGcal(a);ok++}catch(e){}}
        saveStateAfterPaint();toast(`${ok}건 업로드 완료`);
      }
      /* 자동 pull (연결된 경우에만, 특정 캘린더 선택된 경우에만) */
      async function autoPullFromGcal(){
        if(!isConnected())return;
        if(!state.gcalCalendarId)return;
        try{
      ...
      function updateGcalBtn(){
        ...
        updateAssignGcalBtn();renderGcalBanner();
      }
      function updateAssignGcalBtn(){
        const btn=$("#assignGcalConnectBtn");if(!btn)return;
        if(!clientId()){btn.textContent="☁ 구글 연동";btn.style.cssText="font-size:12px";btn.className="btn";return}
        if(isConnected()){btn.textContent="☁ 연결됨 ✓";btn.style.cssText="font-size:12px;background:#0d9488;border-color:#0d9488;color:#fff";btn.className="btn"}
        else{btn.textContent="☁ 재연결 필요";btn.style.cssText="font-size:12px;background:#ef4444;border-color:#ef4444;color:#fff;font-weight:700";btn.className="btn"}
      }
      function renderGcalBanner(){
        ...
        if(!banner){
          panel.insertAdjacentHTML("afterbegin",`<div id="gcalDisconnectBanner" style="background:#f0f9ff;border:1px solid #bae6fd;border-radius:10px;padding:10px 16px;margin-bottom:10px;display:flex;align-items:center;justify-content:space-between;gap:12px"><span style="font-size:13px;color:#0369a1">📅 구글 캘린더 — 클릭 한 번으로 연결하세요</span><button id="gcalReconnectBannerBtn" style="background:#0ea5e9;color:#fff;border:0;border-radius:8px;padding:6px 16px;font-size:13px;cursor:pointer;white-space:nowrap;font-weight:600">연결</button></div>`);
          $("#gcalReconnectBannerBtn").onclick=async()=>{if(await requestToken(true)){toast("✅ 구글 캘린더 연결됐습니다.");updateGcalBtn();autoPullFromGcal();}};
        }
      }
      /* 할일 툴바 버튼 주입 */
      function injectGcalButtons(){
        ...
        const toolbar=$("#todoBoardPanel .todo-toolbar");
        if(!toolbar||toolbar.querySelector("#gcalConnectBtn"))return;
        toolbar.insertAdjacentHTML("beforeend",`<span id="gcalBtnGroup" style="margin-left:auto;display:flex;gap:4px"><button id="gcalConnectBtn" class="btn" style="font-size:12px">☁ 구글 캘린더 연동</button><button id="gcalPullBtn" class="btn" style="font-size:12px" title="오늘 이후 구글 캘린더 → 할일 가져오기">↓ 가져오기</button><button id="gcalPullRangeBtn" class="btn" style="font-size:12px" title="날짜 지정해서 가져오기">📅 기간 가져오기</button><button id="gcalPushBtn" class="btn" style="font-size:12px" title="모든 할일 → 구글 캘린더 업로드">↑ 전체 업로드</button></span>`);
        updateGcalBtn();
      }
      /* 일정관리 캘린더 툴바 버튼 주입 */
      function injectAssignmentGcalButtons(){
        if(currentView!=="assignments")return;
        const toolbar=document.querySelector("#assignmentCalendarPanel .schedule-right");
        if(!toolbar||toolbar.querySelector("#assignGcalConnectBtn"))return;
        toolbar.insertAdjacentHTML("afterbegin",`<span id="assignGcalBtnGroup" style="display:flex;gap:4px;margin-right:8px"><button id="assignGcalConnectBtn" class="btn" style="font-size:12px">☁ 구글 연동</button><button id="assignGcalPushBtn" class="btn" style="font-size:12px" title="일정 전체 구글 캘린더에 업로드">↑ 전체 업로드</button></span>`);
        updateAssignGcalBtn();
      }
      /* renderTodoBoard 래핑 */
      ...
      /* renderAssignmentCalendar 래핑 */
      const baseRenderAssignCalForGcal=renderAssignmentCalendar;
      renderAssignmentCalendar=function(){baseRenderAssignCalForGcal();setTimeout(()=>{injectAssignmentGcalButtons();},0)};
      ...
      /* saveAssignmentBtn 래핑: 저장 후 구글 동기화 */
      const prevSaveAssignment=$("#saveAssignmentBtn")?.onclick;
      if(prevSaveAssignment){
        $("#saveAssignmentBtn").onclick=function(){
          const wasEditing=editingAssignmentIndex;
          prevSaveAssignment.call(this);
          if(!isConnected())return;
          const saved=wasEditing===null?state.assignments[0]:state.assignments[wasEditing];
          if(saved)syncAssignmentToGcal(saved);
        }
      }
      /* googleCalendarBtn: 연결됐으면 API 직접 등록, 아니면 URL 열기 */
      const prevGcalUrlBtn=$("#googleCalendarBtn")?.onclick;
      if(prevGcalUrlBtn){
        $("#googleCalendarBtn").onclick=function(){
          if(isConnected()){
            const a=currentAssignmentForm();
            syncAssignmentToGcal(a).then(()=>toast("✅ 구글 캘린더에 등록됐습니다!"));
          }else{
            prevGcalUrlBtn.call(this);
          }
        }
      }
      ...
      /* deleteAssignmentAt 래핑: 삭제 전 구글에서도 제거 */
      const baseDeleteAssignmentAtForGcal=deleteAssignmentAt;
      deleteAssignmentAt=function(i){const a=state.assignments[i];if(a?.gcalEventId)removeAssignmentFromGcal(a.gcalEventId);return baseDeleteAssignmentAtForGcal(i)};
      ...
      normalizeState=function(){
        ...
        if(!state.gcalToken)state.gcalToken="";
        if(!state.gcalTokenExpiry)state.gcalTokenExpiry=0;
      ...
      document.addEventListener("click",async e=>{
        const t=e.target.closest("button")||e.target;
        if(t.id==="gcalConnectBtn"){e.preventDefault();e.stopImmediatePropagation();if(isConnected()){if(confirm("Google 캘린더 연결을 해제할까요?"))disconnect()}else{if(await requestToken())toast("Google 캘린더에 연결됐습니다.");updateGcalBtn()}return}
        if(t.id==="gcalPullBtn"){e.preventDefault();e.stopImmediatePropagation();pullFromGcal();return}
        if(t.id==="gcalPullRangeBtn"){e.preventDefault();e.stopImmediatePropagation();pullWithDatePicker();return}
        if(t.id==="gcalPushBtn"){e.preventDefault();e.stopImmediatePropagation();if(confirm(`할일 전체(${state.todos.filter(x=>x.status!=="취소").length}건)를 구글 캘린더에 업로드할까요?`))pushAllToGcal();return}
        if(t.id==="assignGcalConnectBtn"){e.preventDefault();e.stopImmediatePropagation();if(isConnected()){if(confirm("Google 캘린더 연결을 해제할까요?"))disconnect()}else{if(await requestToken())toast("Google 캘린더에 연결됐습니다.");updateAssignGcalBtn()}return}
        if(t.id==="assignGcalPushBtn"){e.preventDefault();e.stopImmediatePropagation();if(confirm(`일정 전체(${state.assignments.filter(x=>x.status!=="취소"&&x.status!=="보류").length}건)를 구글 캘린더에 업로드할까요?`))pushAllAssignmentsToGcal();return}
      },true);
      /* 스타일 */
      document.head.insertAdjacentHTML("beforeend",`<style id="gcalStyle">#gcalBtnGroup,#assignGcalBtnGroup{flex-shrink:0}@media(max-width:600px){#gcalBtnGroup,#assignGcalBtnGroup{display:none!important}}</style>`);
    })();
...
function detectSchedType(text){
  const t=(text||'').toLowerCase();
  if(/외근|현장방문|출장/.test(t))return{type:'외근',color:'#ef4444',icon:'🚗'};
  if(/한전|연계|계통/.test(t))return{type:'한전연계',color:'#f59e0b',icon:'⚡'};
  if(/준공|인허가|허가/.test(t))return{type:'인허가/준공',color:'#8b5cf6',icon:'📋'};
  if(/구조물|검수/.test(t))return{type:'구조물검수',color:'#06b6d4',icon:'🔩'};
  if(/회의|미팅|협의/.test(t))return{type:'회의',color:'#3b82f6',icon:'👥'};
  if(/보고|보고서/.test(t))return{type:'보고',color:'#10b981',icon:'📊'};
  if(/시공|공사/.test(t))return{type:'시공',color:'#f97316',icon:'🏗'};
...
function renderSchedule(){
  ...
  const meetings=(state.meetings||[]).filter(m=>m.date).map(m=>({
    id:m.id,title:m.title||'회의',dueDate:m.date,_src:'meet'
  }));
  const allItems=[...todos,...constructions,...meetings];
  const 외근Items=allItems.filter(i=>/외근|현장방문|출장/.test(i.title||i.content||''));
  ...
  const typeStats={};
  monthItems.forEach(i=>{const{type}=detectSchedType(i.title||i.content||'');typeStats[type]=(typeStats[type]||0)+1;});
  const dows=['일','월','화','수','목','금','토'];
  let calCells=dows.map((d,i)=>`<div class="sched-dow ${i===0?'sun':i===6?'sat':''}">${d}</div>`).join('');
  for(let i=0;i<startDow;i++)calCells+='<div class="sched-cell empty"></div>';
  for(let day=1;day<=lastDay.getDate();day++){
    const dateStr=`${year}-${String(month+1).padStart(2,'0')}-${String(day).padStart(2,'0')}`;
    const isToday=today.getFullYear()===year&&today.getMonth()===month&&today.getDate()===day;
    const dayItems=allItems.filter(i=>(i.dueDate||i.date||'').startsWith(dateStr));
    const dow=(startDow+day-1)%7;
    calCells+=`<div class="sched-cell ${isToday?'today':''} ${dow===0?'sun':dow===6?'sat':''} ${dayItems.length?'sched-date-cell-hasitem':''}" data-date="${dateStr}" onclick="openSchedDay('${dateStr}')">
      <div class="sched-day-num">${day}</div>
      ${dayItems.slice(0,3).map(i=>{const{color,icon}=detectSchedType(i.title||i.content||'');return`<div class="sched-item" style="background:${color}20;border-left:3px solid ${color}">${icon} ${(i.title||i.content||'').substring(0,10)}</div>`;}).join('')}
      ${dayItems.length>3?`<div class="sched-more">+${dayItems.length-3}개</div>`:''}
    </div>`;
  }
  const statsHtml=Object.entries(typeStats).map(([type,count])=>{const{color,icon}=detectSchedType(type);return`<div class="sched-stat-chip" style="border-color:${color};color:${color}">${icon} ${type} <strong>${count}</strong></div>`;}).join('')||'<span style="color:#9ca3af">이번 달 일정 없음</span>';
  const 외근Html=외근Items.length===0?'<p class="sched-empty-msg">제목에 "외근","현장방문","출장" 포함된 항목이 없습니다.</p>':
    `<div class="sched-list">${외근Items.sort((a,b)=>((a.dueDate||a.date||'')>(b.dueDate||b.date||''))?1:-1).map(i=>`<div class="sched-list-item" style="border-left-color:#ef4444"><span class="sched-date">${(i.dueDate||i.date||'').substring(0,10)}</span><span class="sched-type-icon">🚗</span><span class="sched-title">${i.title||i.content||''}</span><span class="sched-type-label" style="background:#fef2f2;color:#ef4444">외근</span></div>`).join('')}</div>`;
  const listHtml=monthItems.length===0?'<p class="sched-empty-msg">이번 달 등록된 일정이 없습니다.</p>':
    `<div class="sched-list">${monthItems.sort((a,b)=>((a.dueDate||a.date||'')>(b.dueDate||b.date||''))?1:-1).map(i=>{const{type,color,icon}=detectSchedType(i.title||i.content||'');return`<div class="sched-list-item" style="border-left-color:${color}"><span class="sched-date">${(i.dueDate||i.date||'').substring(5,10)}</span><span class="sched-type-icon">${icon}</span><span class="sched-title">${i.title||i.content||''}</span><span class="sched-type-label" style="background:${color}20;color:${color}">${type}</span></div>`;}).join('')}</div>`;
  ...
  if(els.fieldworkView)els.fieldworkView.classList.add('hidden');
  if(els.mainGrid)els.mainGrid.classList.add('hidden');
  let sv=document.getElementById('schedView');
...
function openSchedDay(dateStr){
  ...
  const items=allItems.filter(i=>(i.dueDate||i.date||'').startsWith(dateStr));
...
function selectSchedType(btn){
  ...
  const ti=document.getElementById('schedTitle');
  if(ti&&!ti.value.trim())ti.value=btn.dataset.type+' - ';
  ti?.focus();
}
...
function saveSchedItem(){
  ...
  const priority=document.getElementById('schedPriority')?.value||'보통';
  ...
  persistState();
  closeModal('schedAddModal');
  toast('✅ 일정이 추가되었습니다.');
  renderSchedule();
}

/* ════════════════════════════════════════════════════
   할일·일정 통합 뷰 (보드 / 목록 / 캘린더 토글)
   - Bug fix: dueDate 필드 → due 필드 정규화
   - 네비게이션 "할일관리" + "일정관리" → "할일·일정" 1개로 통합
════════════════════════════════════════════════════ */
;(function initUnifiedTodoView(){

  // 1. 버그 수정: saveSchedItem()이 만든 todo의 dueDate → due 정규화
  const _origNormalizeTodo = normalizeTodo;
  normalizeTodo = function(t){
    const r = _origNormalizeTodo(t);
    if(r.dueDate && !r.due) r.due = r.dueDate;
    return r;
  };

  // 기존 저장된 todos도 즉시 정규화
  (state.todos||[]).forEach(t=>{ if(t.dueDate && !t.due) t.due = t.dueDate; });

  // 2. localStorage에 fieldwork가 저장된 경우 todos로 리다이렉트
  if(currentView==="fieldwork"){
    todoViewMode="calendar";
    currentView="todos";
    localStorage.setItem(viewStorageKey,"todos");
  }

  // 2b. normalizeState 오버라이드: "할일관리" 재삽입 방지 + 항상 "할일·일정"으로 통합
  // normalizeState는 "할일관리"가 없으면 강제 삽입하므로, 실행 후 정리
  const _baseNS = normalizeState;
  normalizeState = function(){
    _baseNS();
    if(!state?.nav) return;
    const hasUnified = state.nav.some(n=>n.label==="할일·일정");
    if(hasUnified){
      // normalizeState가 "할일관리"를 재삽입했으면 제거
      for(let i=state.nav.length-1;i>=0;i--){
        if(state.nav[i].label==="할일관리") state.nav.splice(i,1);
      }
    } else {
      // "할일관리" → "할일·일정" 이름 변경
      const tn = state.nav.find(n=>n.label==="할일관리");
      if(tn){ tn.label="할일·일정"; tn.icon="☑"; }
    }
    // 항상 "일정관리" 제거
    for(let i=state.nav.length-1;i>=0;i--){
      if(state.nav[i].label==="일정관리") state.nav.splice(i,1);
    }
  };

  // 3. viewForLabel 확장: "할일·일정" → todos
  const _baseVFL = viewForLabel;
  viewForLabel = function(label){
    if(label.includes("할일·일정")) return "todos";
    return _baseVFL(label);
  };

  // 4. isActive 확장: todos 뷰일 때 "할일·일정" 활성화
  const _baseIA = isActive;
  isActive = function(label){
    if(label.includes("할일·일정")) return currentView==="todos";
    return _baseIA(label);
  };

  // 5. 캘린더 상태
  let calY = new Date().getFullYear(), calM = new Date().getMonth();

  // 6. 인라인 캘린더 렌더러 (todoBoardPanel 안에 렌더링)
  function renderTodoCal(){
    const panel = document.getElementById("todoBoardPanel");
    if(!panel) return;
    panel.classList.remove("hidden");
    const acp = document.getElementById("assignmentCalendarPanel");
    ...
    const startDow = firstDay.getDay();
    const mpfx = `${calY}-${String(calM+1).padStart(2,'0')}`;

    // 날짜 필드를 통합 (_due = due || dueDate || date)
    const allTodos = (state.todos||[]).map((t,i)=>({...t, _i:i, _due: t.due||t.dueDate||t.date||''}));
    const monthTodos = allTodos.filter(t=>t._due.startsWith(mpfx));

    const sc = {'할 일':'#3b82f6','진행중':'#f59e0b','완료':'#10b981','백로그':'#94a3b8','취소':'#ef4444','진행전':'#6366f1'};

    const dows=['일','월','화','수','목','금','토'];
    let cal = dows.map((d,i)=>`<div class="sched-dow ${i===0?'sun':i===6?'sat':''}">${d}</div>`).join('');
    for(let i=0;i<startDow;i++) cal+='<div class="sched-cell empty"></div>';
    for(let day=1;day<=lastDay.getDate();day++){
      const ds=`${mpfx}-${String(day).padStart(2,'0')}`;
      const isToday=ds===todayStr;
      const dow=(startDow+day-1)%7;
      const dayTodos=allTodos.filter(t=>t._due.startsWith(ds));
      cal+=`<div class="sched-cell ${isToday?'today':''} ${dow===0?'sun':dow===6?'sat':''} ${dayTodos.length?'sched-date-cell-hasitem':''}" data-tcal-date="${ds}">
        <div class="sched-day-num">${day}</div>
        ${dayTodos.slice(0,3).map(t=>{const c=sc[t.status]||'#64748b';return`<div class="sched-item" style="background:${c}18;border-left:2px solid ${c}">${esc((t.title||'').substring(0,10))}</div>`;}).join('')}
        ${dayTodos.length>3?`<div class="sched-more">+${dayTodos.length-3}개</div>`:''}
      </div>`;
    }

    const sorted=[...monthTodos].sort((a,b)=>a._due>b._due?1:-1);
    const listHtml=sorted.length===0
      ?'<p style="color:#9ca3af;text-align:center;padding:20px 0;font-size:13px">이번 달 등록된 할일이 없습니다.</p>'
      :`<div class="sched-list">${sorted.map(t=>{const c=sc[t.status]||'#64748b';return`<div class="sched-list-item" style="border-left-color:${c};cursor:pointer" data-edit-todo="${t._i}"><span class="sched-date">${t._due.substring(5,10)}</span><span class="sched-title" style="flex:1">${esc(t.title||'')}</span><span class="sched-type-label" style="background:${c}18;color:${c};border-radius:999px;padding:2px 8px;font-size:11px;white-space:nowrap">${esc(t.status||'')}</span></div>`;}).join('')}</div>`;

    panel.innerHTML=`
      <div class="todo-toolbar">
        <div class="todo-view">
          <button data-todo-view="board">보드</button>
          <button data-todo-view="list">목록</button>
          <button class="active" data-todo-view="calendar">캘린더</button>
        </div>
        <button class="btn primary" id="todoAddBtn">할일 추가</button>
      </div>
      <div>
        <div class="sched-header" style="margin:10px 0 8px">
          <div class="sched-nav">
            <button class="btn icon" id="tcalPrev">◀</button>
            <h2 style="font-size:1rem;margin:0;min-width:80px;text-align:center">${calY}년 ${calM+1}월</h2>
            <button class="btn icon" id="tcalNext">▶</button>
            <button class="btn" id="tcalToday" style="font-size:12px;padding:0 10px">오늘</button>
          </div>
        </div>
        <div class="sched-calendar" style="margin-bottom:14px">${cal}</div>
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
          <strong style="font-size:.9rem;color:#1f2937">${calM+1}월 할일</strong>
          <span class="sched-badge">${monthTodos.length}건</span>
        </div>
        ${listHtml}
      </div>`;

    panel.querySelector('#tcalPrev')?.addEventListener('click',()=>{calM--;if(calM<0){calM=11;calY--;}renderTodoCal();});
    panel.querySelector('#tcalNext')?.addEventListener('click',()=>{calM++;if(calM>11){calM=0;calY++;}renderTodoCal();});
    panel.querySelector('#tcalToday')?.addEventListener('click',()=>{calY=new Date().getFullYear();calM=new Date().getMonth();renderTodoCal();});
    panel.querySelectorAll('[data-tcal-date]').forEach(cell=>{
      cell.addEventListener('click',e=>{
        if(e.target.closest('[data-edit-todo]')) return;
        const date=cell.dataset.tcalDate;
        openTodoModal(null,'할 일');
        setTimeout(()=>{
          const df=document.getElementById('todoDue'),sf=document.getElementById('todoStart');
          if(df) df.value=date; if(sf) sf.value=date;
        },50);
      });
    });
  }

  // 7. renderCurrentContent 오버라이드: calendar 모드 처리
  const _baseRCC = renderCurrentContent;
  renderCurrentContent = function(){
    if(currentView==="todos" && todoViewMode==="calendar"){ renderTodoCal(); return; }
    _baseRCC();
  };

  // 8. renderTodoBoard 오버라이드: 캘린더 탭 삽입
  const _baseRTB = renderTodoBoard;
  renderTodoBoard = function(){
    if(todoViewMode==="calendar"){ renderTodoCal(); return; }
    _baseRTB();
    const tv=document.querySelector('.todo-view');
    if(tv && !tv.querySelector('[data-todo-view="calendar"]')){
      const b=document.createElement('button');
      b.dataset.todoView='calendar'; b.textContent='캘린더';
      tv.appendChild(b);
    }
  };

  // 9. 헤더 타이틀을 "할일·일정"으로 업데이트
  const _baseRender = render;
  render = function(){
    _baseRender();
    if(currentView==="todos"){
      const pt=document.getElementById('pageTitle')||document.querySelector('h1.page-title');
      if(pt) pt.textContent="할일·일정";
    }
  };
